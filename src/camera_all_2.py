...
while True:
    ...
    if (distanceTF1 < 6.0 and distanceTF1 > 2.0) or (distanceTF2 < 6.0 and distanceTF2 > 2.0): # button is pressed to turn on the system
        ...
        #label = "cat"
        #led.color = (0, 0, 1)
        if distanceTF1 < 6.0 and distanceTF1 > 2.0: # distanceUD < 0.5 and (for distance sensor reliability)
        ...
        if distanceTF2 < 6.0 and distanceTF2 > 2.0:
              # Camera checking code here
             image = picam2.capture_array()
             counter += 1
             image = cv2.flip(image, -1)
